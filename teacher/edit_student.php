...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    file_put_contents('edit_student_debug.log', 'POST Data: ' . print_r($_POST, true) . PHP_EOL, FILE_APPEND);
    ...
    if (empty($last_name) || empty($first_name) || empty($sex) || empty($email)) {
        ...
        if ($result_check_email->num_rows > 0) {
            ...
            file_put_contents('edit_student_debug.log', "Attempting to update student_id: $student_id" . PHP_EOL, FILE_APPEND);
            ...
            if ($stmt_update->execute()) {
                file_put_contents('edit_student_debug.log', 'Affected Rows: ' . $stmt_update->affected_rows . PHP_EOL, FILE_APPEND);
                if ($stmt_update->affected_rows > 0) {
                    ...
                    $error = "Failed to update student information. The data might be unchanged or the student record could not be found.";
                }
            } else {
                $error = "Error executing the update query. Please try again.";
            }
        }
    }
}
